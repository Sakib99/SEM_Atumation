...
def main():
    ...
    target = obj.spec['target'] # generate_target_sem
    ...
    loc =  obj.get_sql_query_location(target)
    sql = obj.prepare_sql(loc=loc, sql_args=sql_args)
    print(sql)
    obj.bq_to_bq(project=obj.spec['project'], dataset=args.dataset, table=obj.spec['target'], credentials="none", sql=sql)
    print("Big_Query_For the target SEM")
    obj.bq_to_gcs(project=obj.spec['project'], dataset=args.dataset, table=obj.spec['target'], bucket=obj.spec['bucket'], folder=obj.spec['folder'], filename=obj.spec['target'] + '.csv', credentials="none")
